...
keggUI <- function(id) {
  ...
  tagList(
    bs4Dash::tabsetPanel(
      tabPanel(title = 'Data',
               fluidRow(
                 ...
                 box(width = 3,status="success",
                     ...
                     fluidRow(
                       column(width = 6,selectInput(inputId = ns("ID"),h6(lang$t('基因列'), style = "color:orange"),c("")) ),
                     ...
                     selectInput(ns('type'),lang$t('输入数据类型'),selected = 'gene',
      ...
      tabPanel(title = 'KEGG',
               fluidRow(
                 box(title=lang$t("KEGG 分析"),width=9,solidHeader=TRUE,status = "primary",background = "white",
                 ...
                 box(width = 3,status="success",
                     ...
                     selectInput(ns("plot_kegg_id1"), lang$t("图形选择："), choices = c("p_dot","p_bar", "p_emap") ),br(),
      ...
      tabPanel(title = 'Help', kegg_helpUI("go"))
      )
  ) # NS(id)
} # function(id)
