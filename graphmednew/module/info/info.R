  ...
  tagList(
    fluidRow(
      ...
      box(width=5,title="使用说明",solidHeader=TRUE,status='primary',
          ...
          tags$h6("对于差异分析（RNA-seq）以及富集分析（Enrichment），
                  由于网络原因，此网页可能无法成功运行，
                  可参考 GitHub 中说明，用本地电脑运行。"),
          tags$hr(),
          tags$h3("Previous version"),
          tags$a("https://shiny.chcmu.com.cn/oldgraphmed/",href="https://shiny.chcmu.com.cn/oldgraphmed/"),
          tags$hr(),
          # tags$video(src="./video/elisa.mp4",type = "video/mp4",width="80%"),
          tags$h3("使用说明"),
          tags$h6("1、参考参考数据：点击“show data”，或下载各自工具的对应参考数据；"),
          tags$h6("2、上传数据，格式为 csv/xlsx/xls。csv 文件编码格式为 “GB18030”。"),
          tags$h6("3、点击“Analysis”/“Start drawing”进行分析。。"),
          tags$h6("4、调节分析或图形参数。"),
          tags$h6("5、地图可不用下载参考数据，直接点击运行后修改示例数据即可。"),
          tags$h6("6、其他图形，更新中......")
      )
    )  )
