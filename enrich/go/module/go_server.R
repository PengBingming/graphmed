              ...
              observe({ 
                ...
                kk <- reactive({
                  ...
                  if(input$type=='DEG'){
                    ...
                    # 2.1 差异基因富集
                    kk <- clusterProfiler::enrichGO(gene          = gene_kegg ,
                                   ont           = "ALL", # "BP" "CC" "MF"
                                   OrgDb         = OrgDb.db() ,
                ...
                observe({
                  ...
                  if(input$plot_kegg_id1 == "p_emap"){
                    ...
                    updateSelectizeInput(session,inputId = 'kegg_num1', server = T, # label = '通路选择', 
                                         choices  = kk_emap@result[["Description"]],
                                         selected = kk_emap@result[["Description"]][1:5])
                    }
                  else if(!input$plot_kegg_id1 == "p_emap"){
                    updateSelectizeInput(session,inputId = 'kegg_num1',server = T, # label = '通路选择',
                                         choices  = kk@result[["Description"]],
                                         selected = kk@result[["Description"]][1:5])
                  }
                  updateSelectizeInput(session,inputId = 'kegg_kk',server = T, # label =   '通路选择',
                                       selected = kk@result[["Description"]],
                                       choices = kk@result[["Description"]][1])
                  } ) 
