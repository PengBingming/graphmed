...
errorbarUI <- function(id) {
  ...
  bs4Dash::tabsetPanel(
    tabPanel(title = 'Data',
             fluidRow(
               ...
               box(width = 3,status="success",
                   ...
                   column(width = 12,selectInput(ns('na_omit'), lang$t("na.omit"), c(T,F),selected = T )),
                   hr(),
    ...
    tabPanel(title = 'Result',
             fluidRow(
               box(title=lang$t("Result"),width=9,solidHeader=TRUE,status='primary',background = "white",
                   splitLayout(cellWidths = c("100%"), rHandsontableOutput(ns("resultTable") ) ) ),
               box(width = 3,status="success",
                   actionBttn( inputId = ns("result"), label = lang$t("Calculate"),
                               style = "fill", color = "primary", size = "sm" ),
                   hr(),
                   column(width = 12,selectInput(ns('group1'),h6( "group1: factor",style="color:orange"), c(""),multiple = F )),
                   column(width = 12,selectInput(ns('group2'),h6( "group2: numeric",style="color:orange"), c(""),multiple = F )),
                   column(width = 12,selectInput(ns('var_select'), h6("var.select: numeric",style="color:orange"), c(""), multiple = T )),
                   hr(),
                   downloadButton(ns("downloadResult"),lang$t("Result"))
               )
             )
    ),
    tabPanel(title = 'Plot',
             fluidRow(
               box(width=9, title=lang$t("Plot"),solidHeader=TRUE,status = "primary",background = "white",
                   fluidRow(
                     column(width =3,
                            dropdownButton(circle=FALSE, label= lang$t("basic"),size = "sm", br(),br(),
                                           fluidRow(
                                             ...
                                             column(width = 6, textInput(ns('title'), label = "plot.title", value = "" )),
               ...
               box(width=3,status="success",
                   ...
                   fluidRow(
                     column(width = 6, selectInput(ns("x.group1"), label = 'group1', c("") ) ),
                     column(width = 6, selectInput(ns("x.group2"), label = 'group2', c("") ) ),
                     column(width = 6, selectInput(ns("y"), label = 'mean', c("") ) ),
                     column(width = 6, selectInput(ns("color"), label = 'color', c("") ))
                     # column(width = 6, selectInput(ns("fill"), label = 'fill', c("") ) )
                   ),
                   dropdownButton(circle=FALSE, label = "point & line", br(),br(),
                                  fluidRow(
                                    column(width = 6,numericInput(ns("size_line"), label =  'line.size', value = 1) ),
                                    column(width = 6,numericInput(ns("alpha_line"), label =  'line.alpha', value = 0.5,min = 0,max = 1) ),
                                    column(width = 6,numericInput(ns("size_point"), label =  'point.size', value = 1) ),
                                    column(width = 6,numericInput(ns("alpha_point"), label =  'point.alpha', value = 0.5,min = 0,max = 1) )
                                    # column(width = 6, selectInput(ns("show.legend"),label =  'show.legend',
                                    #                             c("T","F"), selected = 'T' ) )
                                    )
                   ),br(),
                   dropdownButton(circle=FALSE, label = "errorbar", br(),br(),
                                  ...
                                  column(width = 6,numericInput(ns("dodge"), label =  'position.dodge', value = 0.1,min = 0,max = 1) )
                                  )
                   ),
                   hr(),
