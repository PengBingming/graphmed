    ...
    tabPanel(title = 'Plot',
             fluidRow(
                     ...
                     column(width =3,
                            dropdownButton(circle=FALSE, label= lang$t("basic"),size = "sm", br(),br(),
                                           fluidRow(
                                             ...
                                             column(width = 6, textInput(ns('title'), label = "plot.title", value = "ggplot" )),
               ...
               box(width=3,status="success",
                   ...
                   br(),
                   selectInput(ns("plotType"),  lang$t('Graphic selection'),selected = "violin",
                               c("point","jitter","boxplot","violin","histogram","density",
                                 "dotplot","text"),multiple = T ),
                   # violin
                   if(T){
                     conditionalPanel(
                     ...
                     conditionalPanel(
                       ...
                       dropdownButton(circle=FALSE, label='jitter', br(),br(),
                                            fluidRow(
                                              ...
                                              column(width = 6,numericInput(ns("size_jitter"), 'size', value = 1 ) ),
                                              column(width = 6,numericInput(ns("shape_jitter"), 'shape',value = 21) ),
                   ...
                   if(T){
                     conditionalPanel(
                       condition = "input.plotType.includes('histogram')",ns = NS(id),
                       dropdownButton(circle=FALSE, label='histogram', br(),br(),
                                           fluidRow(
                                             column(width = 6,selectInput(ns("x_histogram"), 'x', c("") ) ),
                                             column(width = 6,selectInput(ns("color_histogram"), 'color', c("") ) ),
                                             column(width = 6,selectInput(ns("fill_histogram"), 'fill', c("") ) ),
                                             column(width = 6,numericInput(ns("alpha_histogram"), 'alpha', value = 0.5 )),
                                             column(width = 6,numericInput(ns("size_histogram"), 'size', value = 1 )),
                                             column(width = 6,numericInput(ns("bins_histogram"), 'bins', value = 10 )),
                                             column(width = 6,numericInput(ns("binwidth_histogram"), 'binwidth', value = 1 )),
                                             column(width = 6,selectInput(ns("show.legend_histogram"), 'show.legend', 
                                                                          c("T","F"),selected = 'F'  ) )
                                           )
                   ...
                   if(T){
                     conditionalPanel(
                       condition = "input.plotType.includes('dotplot')",ns = NS(id),
                       dropdownButton(circle=FALSE, label='dotplot', br(),br(),
                                           fluidRow(
                                             column(width = 6,selectInput(ns("x_dotplot"), 'x', c("") ) ),
                                             column(width = 6,selectInput(ns("color_dotplot"), 'color', c("") ) ),
                                             column(width = 6,selectInput(ns("fill_dotplot"), 'fill', c("") ) ),
                                             column(width = 6,selectInput(ns("method_dotplot"), 'method', c("dotdensity","histodot") ) ),
                                             column(width = 6,selectInput(ns("stackdir_dotplot"), 'stackdir',selected = 'up', 
                                                                          choices = c("up","down","center","centerwhole") ) ),
                                             column(width = 6,numericInput(ns("alpha_dotplot"), 'alpha', value = 0.5 )),
                                             column(width = 6,numericInput(ns("dotsize_dotplot"), 'dotsize', value = 1 )),
                                             column(width = 6,numericInput(ns("linetype_dotplot"), 'linetype',value = 21)),
                                             column(width = 6,numericInput(ns("binwidth_dotplot"), 'binwidth',value =1)),
                                             column(width = 6,selectInput(ns("show.legend_dotplot"), 'show.legend', 
                                                                          c("T","F"),selected = 'F'  ) )
                                           )
                       ) , br() ) },
                   # text
