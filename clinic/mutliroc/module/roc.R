...
roc_helpUI <- function(id) {
  ns <- NS(id)
  tagList(
    fluidRow(box(width=12,title="使用说明",solidHeader=TRUE,status='primary',background = "white",height="100%",
                 tags$h2("使用说明"),
                 tags$hr(),
                 tags$h6(lang$t("1、参考数据中，首先选择结局变量。")),
                 tags$h6(lang$t("2、选择变量可选择多个。")),
                 tags$h6(lang$t("3、当选择多个变量时，设置合并变量 
                                 no 表示每个变量单独预测结局后图形合并。
                                 yes 表示变量联合对结局的预测。"))
...
rocUI <- function(id) {
  ...
  tagList(
    ...
    bs4Dash::tabsetPanel(
      ...
      tabPanel(title = 'Plot',
               fluidRow(
                 ...
                 box(width=3,status="success",
                     ...
                     fluidRow(
                       column(width = 6,selectInput(ns("outcome"),   lang$t("结局"), c("") ) ),
                       column(width = 6, selectInput(ns("merge.var"),  lang$t("合并变量"), c("yes","no"),
                                                     selected = 'no') ),
                       column(width = 12,selectInput(ns("var"), lang$t("选择变量"), c(""),multiple = T ) )
                       
                     ),
                     br(),
                     dropdownButton(circle=FALSE, label=lang$t("图形标签"),icon = icon('image'),  br(),br(),
                                    fluidRow(
                                      column(width = 6,selectInput(ns('point'),label = lang$t('点'),selected = 'show',
                                                                   choices = c('show','hide') ) ),
                                      column(width = 6,selectInput(ns('eta'),label = 'lr.eta',selected = 'show',
                                                                   choices = c('show','hide') ) ) ,
                                      column(width = 6,selectInput(ns('sen'),label = lang$t('灵敏度'),selected = 'show',
                                                                   choices = c('show','hide') ) ),
                                      column(width = 6,selectInput(ns('auc'),label = "AUC",selected = 'show',
                                                                   choices = c('show','hide') ) ),
                                      column(width = 6,selectInput(ns('facet'),label = lang$t('分面'), selected = 'column',
                                                                   choices = c('none','row','column') ) ),
                                      column(width = 6, selectInput(ns("color_type"), label = 'color.theme', color_type ) ),
                                      column(width = 6,textInput(ns("color_name"),label = "legend.name",value = "") ),
                                      column(width = 6,selectInput(ns("legend.position"),label = "legend.position",selected = "none",
      ...
      tabPanel(title = 'Help',roc_helpUI("heatmap"))
    ) ) }
        ...
        observeEvent(input$submit, {
          ...
          plot <- reactive({
            ...
            if(length(input$var)==1 ){
              ...
              if( input$merge.var=='no'& !input$legend.position=="none"){
                p <- p+
                  scale_colour_discrete( name   = input$color_name,
